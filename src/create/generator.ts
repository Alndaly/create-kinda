...
export class Generator {
	...
	// 添加加载动画
	async wrapLoading(fn: any, message: string, ...args: any) {
	...
	async getRepo(filter: string) {
		// 1）从远程拉取模板数据
		const repoList = await this.wrapLoading(getRepoList, '等待获取模版');
		if (!repoList) return;
		console.log(this.structure)
		if (this.structure === 'vite') {
			console.log(`Kinda cli based on Vite is still under development, please wait for some time, or connect ${chalk.cyan(
				'1142704468@qq.com')}`)
		...
		if (!repos.length) {
			console.log(
				`There is no templates on the git, please waiting for developing, or connect ${chalk.cyan(
					'1142704468@qq.com'
				)
				} `
			);
	...
	async create() {
		try {
			...
			// 4）模板使用提示
			console.log(
				`${chalk.green('Successfully')} create ${chalk.cyan(this.name)}`
			);
